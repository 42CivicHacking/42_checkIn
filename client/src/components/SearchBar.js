...
const SearchBar = forwardRef((props, ref) => {
  ...
  const gaepoCard = [
    1, 2, 3, 5, 6, 8, 10, 11, 13, 14, 15, 16, 19, 20, 21, 23, 25, 26, 29, 34, 35, 36, 37, 38, 40,
    41, 42, 43, 44, 45, 46, 47, 50, 51, 52, 53, 54, 55, 57, 58, 59, 63, 64, 65, 66, 68, 69, 70, 71,
    72, 73, 74, 76, 78, 79, 80, 81, 85, 86, 87, 89, 90, 91, 92, 93, 95, 96, 99, 101, 102, 103, 104,
    106, 108, 110, 111, 112, 113, 115, 116, 117, 118, 122, 123, 124, 125, 129, 130, 132, 133, 135,
    136, 137, 142, 146, 147, 149, 150, 151, 152, 153, 154, 156, 158, 161, 163, 166, 167, 168, 169,
    170, 171, 172, 173, 176, 179, 184, 185, 186, 189, 191, 198, 199, 201, 204, 205, 206, 207, 208,
    210, 212, 213, 214, 215, 216, 218, 219, 220, 222, 225, 226, 228, 229, 231, 233, 234, 235, 240,
    242, 243
  ];
  const seochoCard = [
      ...
      switch (props.type) {
        case 0:
          response = await axios.get(
            `/api/log/${ClusterType === 0 ? 'gaepo' : 'seocho'}?${props.Page}`
          );
          break;
        case 1:
          response = await axios.get(`/api/log/user/${Login}?${props.Page}`);
          break;
        case 2:
          response = await axios.get(`/api/log/card/${CardId}?${props.Page}`);
          break;
        case 3:
          response = await axios.get(`/api/log/checkIn/${ClusterType}?${props.Page}`);
          break;
        case 4:
